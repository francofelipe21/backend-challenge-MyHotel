...
public interface SurveyResultRepository extends JpaRepository<SurveyResponse, Long> {
    ...
    FROM
        guests g
    INNER JOIN
        surveys s ON s.guest_id = g.id and s.id = :surveyResultId
    INNER JOIN
    ...
    */
    ...
    /**
     * This query the amount of user by domain
     */
    @Query(value = """
    SELECT *
    FROM(
    SELECT g.domain, count(*) as count
    from(
    SELECT
        substring(g.email, LOCATE('@', g.email) + 1,  length(g.email)) as domain
    FROM
        guests g) as g
    GROUP BY
        g.domain) as g
    ...
    """, nativeQuery = true)
    List<EmailsOverviewTemplateDataProjection> getEmailsOverview();
}
